...
p1 <- ggplot(data=mites, aes(x=SubsDens, y=Galumna)) + geom_jitter() + geom_abline(intercept = coef(lm.g.subsdens)[1], slope = coef(lm.g.subsdens)[2], color="darkred")
p2 <- ggplot(data=mites, aes(x=WatrCont, y=Galumna)) + geom_jitter() + geom_abline(intercept = coef(lm.g.watrcont)[1], slope = coef(lm.g.watrcont)[2], color="darkred")
...
p1 <- ggplot(data=mites, aes(x=SubsDens, y=pa)) + geom_point() + geom_abline(intercept = coef(lm.pa.subsdens)[1], slope = coef(lm.pa.subsdens)[2], color="darkred")
p2 <- ggplot(data=mites, aes(x=WatrCont, y=pa)) + geom_point() + geom_abline(intercept = coef(lm.pa.watrcont)[1], slope = coef(lm.pa.watrcont)[2], color="darkred")
grid.arrange(p1,p2, nrow=1)
```
...
p1 <- ggplot(data=mites, aes(x=SubsDens, y=prop)) + geom_point() + geom_abline(intercept = coef(lm.prop.subsdens)[1], slope = coef(lm.prop.subsdens)[2], color="darkred")
p2 <- ggplot(data=mites, aes(x=WatrCont, y=prop)) + geom_point() + geom_abline(intercept = coef(lm.prop.watrcont)[1], slope = coef(lm.prop.watrcont)[2], color="darkred")
grid.arrange(p1,p2, nrow=1)
```
...
La grande p-value nous indique que les variables présentent dans le plus grand modèle mais pas dans les plus petit ne sont pas significatives. Avant de confirmer le choix de ce modèle, regardons les valeurs du critères BIC (critère explicatif) pour chacuns des deux modèles.
...
-   Evaluation de l'ajustement du modèle
    ...
    Afin de tester la qualité de l'ajustement de notre modèle, on peut utiliser le test de Pearson basé sur les résidus de Pearson.
    ...
    Nous remarquons deux individus ayant des résidus extrèmes. Une nouvelle fois, nous retrouvons l'individu 15. Il serait peut-être judicieux de l'enlever pour la suite. Cependant, le jeu de données n'étant pas très grand, nous décidons de le conserver afin de ne pas le réduire davantage.

    -   Test d'Hosmer-Lemeshow

    Par ailleurs, on peut également effectuer un test d'Hosmer-Lemeshow pour évaluer la pertinence du modèle.
...
Afin d'évaluer le pouvoir explicatif de notre modèle, on peut calculer son Pseudo-$R^2$. Il en existe plusieurs. Ici, nous allons déterminer celui-ci de McFadden (1973) correspondant au quotient entre la différence des déviances nulles et résiduelles et la déviance nulle.
...
Nous allons commencer par le modèle global et ensuite effectuer une sélection stepwise.
```{r}
glm.tot.poi <- glm(pa~WatrCont + SubsDens + Topo + Shrub + Substrate, data=mites, family = poisson(link='log'))
summary(glm.tot.poi)
```
Nous observons dans le `summary` de notre modèle global qu'il n'y que très peu de variables significatives. On créé un modèle avec les deux variables significatives du modèle complet : `WatrCont` et `SubsDens`.
...
```
Avec pour le modèle 1, le modèle globale, pour le modèle 2, le modèle avec les variables `WatrCont` et `SubsDens` et le modèle 3, le modèle selectionné par la sélection stepwise. 
...
Le modele selectionné s'écrit donc : $$ln(\lambda)=\beta_O+\beta_1*WatrCont+\beta_2*SubsDens+\beta_3*Topo$$ ce qui nous donne à l'aide du `summary` de notre modèle : $$ln(\lambda)=1.89-0.008*WatrCont+0.023*SubsDens + 0.892*Topo $$ Nous allons maintenant interpreter les coefficients de notre modèle. Nous allons determiner l'effet de chaque coefficient du modèle sur le prédicteur linéaire $\mu_i=\beta_O+\beta_1*WatrCont+\beta_2*SubsDens+\beta_3*Topo$, puis de déduire l'effet sur la moyenne de réponse à partir de la fonction $\lambda=e^\eta$.
...
### I. Nous allons simuler $n=100$ réalisations d'un vecteur gaussien $(X,Y)$ de moyenne et de variance et ajouter des données manquantes sur Y selon :
