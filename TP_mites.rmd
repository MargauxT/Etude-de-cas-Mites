...
bc <- boxcox(lm.tot.pa.translat, lambda=seq(-5,2,0.1), plotit = F)
lambda <- bc$x[which.max(bc$y)]
cat("Lambda_1 optimal : ", lambda)
```
...
bc <- boxcox(lm.tot.prop.translat, lambda=seq(-200,0,0.1), plotit = F)
lambda <- bc$x[which.max(bc$y)]
cat("Lambda_1 optimal : ", lambda)
```

On réajuste un modèle sur les données transformées par Box & Cox avec $\lambda_1=-147$. Il s'agit d'une valeur de $\lambda_1$ très élevée comparée aux valeurs précédents, mais c'est celle qui maximise la log-vraisemblance de ??????.
...
Dans la pratique, il existe de nombreuses distributions de probabilité pour décrire les données : Poisson, Bernouilli, Binomiale... Nous allons donc essayer une autre approche pour ajuster un modèle sur nos données : les modèles linéaires généralisés.\
...
## 2. GLM Régression logistique binaire sur la variable d'occurrence (pa)
...
Ici, le modèle linéaire généraliséest composé de :

1.  la variable réponsé $Y_i, i=1,...,n$ qui suit une distribution de Bernouilli de paramètre $\pi_i$
...
summary(glm.tot.pa)
```
La sortie du `summary` nous permet de voir quelles variables sont significatives pour le test de Wald. Celui-ci permet de tester la nullité d'un coefficient : $H_0 : \beta_j=0$. Sous $H_0$, on a :
...
Afin de s'assurer de la significativité du retrait des autres variables, nous pouvons réaliser un test de modèles emboîtés. **DÉFINIR**
...
La sortie de notre modèle indique que le contenu en eau, la densité du substrat et la topographie sont associés significativement à l'occurrence de mites. Cependant, on peut également interpréter les coefficients de la pente grâce aux odds-ratio. Il est défini comme le rapport des probabilités d'apparition de l'évènement $Y=1$ contre $Y=0$. L'odds est la quantité $\frac{\pi_i}{1-\pi_i}$. Dans le modèle logistique, on définit plus précisément l'odds par :
$$\frac{\pi_i}{1-\pi_i}=\exp(x_i^T\beta)=\exp(\beta_0+\beta_1x^i_1+...+\beta_px_i^p)$$ et si on considère deux individus $i_1$ et $i_2$ dont la valeur des covariables ne diffère que de 1 pour la $j$-ième covariable, soit $x_{i_1}^j-x_{i_2}^j=1$, on calcule l'odd-ratio avec :
...
```{r, echo=F}
cat("Odds-ratios : ",exp(glm.best.pa$coefficients))
```
...
## 3. GLM Régression Logistique sur les données agrégées (prop)
...
## 4. GLM Régression Poisson sur la variable d'abondance (Galumna)
...
Le modele selectionné s'écrit donc : $$ln(\lambda)=\beta_O+\beta_1*WatrCont+\beta_2*SubsDens+\beta_3*Topo$$ ce qui nous donne à l'aide du `summary` de notre modèle : $$ln(\lambda)=0.710-0.008*WatrCont+0.034*SubsDens + 0.892*TopoHummock $$
...
Nous pouvons regarder plus en détails comment sont reparties les données manquantes de $Y$ selon les valeurs de $X$ pour chacun des jeux de données.
...
Nous remarquons que pour les deux dernières configurations, les valeurs manquantes sont plutôt centrées autour de $X \simeq 0.75$, tandis que dans le premier cas, il en existe également pour des valeurs beaucoup plus faibles de $X$.
...
# III. Simulation de données manquantes sur le jeu de données `mites`. 
