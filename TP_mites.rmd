...
-   sur les erreurs $\epsilon_i$ qui sont :\
    (H2) **centrées** : $\mathbb{E}(\epsilon_i)=0, \forall i=1,..n$, cette condition est toujours vérifiée par les moindres carrés ordinaires (MCO), technique de résolution de la régression linéaire (H3) **homoscédastiques** : $\mathbb{V}(\epsilon_i)=\sigma^2, \forall i$\
...
p2 <- ggplot(data=mites, aes(Substrate, SubsDens)) + geom_boxplot(color="darkred") + labs(title = "SubsDens ~ Substrate") + theme(plot.title = element_text(size = 10, color = "darkred", face = "bold"))
...
```{r, echo=F}
p1 <- ggplot(data=mites, aes(Substrate, WatrCont)) + geom_boxplot(color="darkred") + labs(title = "WatrCont ~ Substrate") + theme(plot.title = element_text(size = 10, color = "darkred", face = "bold"))
...
```{r, echo=F}
p1 <- ggplot(data=mites, aes(Substrate, fill=Shrub)) + geom_bar() +scale_fill_manual(values=c("#330000", "#990000", "#CC0000")) + labs(title = "Substrate ~ Shrub") + theme(plot.title = element_text(size = 10, color = "darkred", face = "bold"))
p2 <- ggplot(data=mites, aes(Substrate, fill=Topo)) + geom_bar() +scale_fill_manual(values=c("#330000", "#990000", "#CC0000")) + labs(title = "Substrate ~ Topo") + theme(plot.title = element_text(size = 10, color = "darkred", face = "bold"))
grid.arrange(p1, p2, nrow=1)
```
...
### 1.2 Transformations sur les données
...
On voit ci-dessus que la transformation $arcsin$ ne change pas énormément la variable `prop`. Regardons ce que cela provoque sur les hypothèses du modèle linéaire gaussien.\
...
Une nouvelle fois, on rejette $H_O$ et on conclut que l'hypothèse de normalité des résidus n'est toujours pas vérifiée, même après une transformation $arcsin$.

#### Transformation de Box-Cox

TO-DO part 1: *transformation BoxCox rappel, application* sauts de ligne, noms variables trop longs barplot boxplot
...
bc <- boxcox(lm.tot.g.translat, lambda=seq(-3,2,0.1), plotit = F)
lambda <- bc$x[which.max(bc$y)]
cat("Lambda optimal : ", lambda)
```

On réajuste un modèle sur les données transformées par Box & Cox avec $\lambda_1=-1.8$.
...
shapiro.test(lm.tot.g.bc$residuals)
```
...
***NOTE à enlever** : dans les "outils" dont elle parle dans son diapo, la moitié on ne peut pas les utiliser car on a que 2 var et qu'une qui a des valeurs manquantes : par ex gg_miss_var sert à rien avec 2 var car compare juste les pourcentages de Na et gg_miss_upset nécessite au moins deux var avec des Na.*

#### 1.3 Estimation de la moyenne de $Y$ et de l'intervalle de confiance associé
...
Comme évoqué, nous débutons par une analyse des cas complets. Cela consiste à supprimer les observations pour lesquelles la valeur de $Y$ est manquante. C'est la méthode la plus simple. Cependant, elle est déconseillée lorsque le nombre de données manquantes est trop élevé car nous perdrions trop d'informations. De plus, pour les jeux de données où les valeurs manquantes ne sont pas type MCAR, retirer des observations va induire un biais dans l'analyse puisque l'ensemble des observations pour lesquelles des données sont manquantes n'est pas forcément représentatif de l'échantillon initial.
...
La méthode précédente ayant des résultats encourageants, nous souhaitons poursuivre son étude. En effet, une valeur unique ne pouvant pas refléter l'incertitude sur la prédiction, nous allons tester de renouveller vingt fois l'imputation.
