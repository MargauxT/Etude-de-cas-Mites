...
Comme décrit ci-dessus, certaines variables sont des facteurs, mais le logiciel R ne les interprète pas comme cela naturellement. Nous allons donc les transformer en facteurs manuellement.
...
```{r, echo=F}
ggplot(data = mites) + geom_bar(mapping = aes(Galumna), fill='darkred') +labs(y = "Nombre de coeurs", x = "Nombre de Galumna", title = "Distribution de Galumna")
```
...
```{r, echo=F}
ggplot(data = mites) + geom_bar(mapping = aes(pa), fill='darkred') +labs(y = "Nombre de coeurs", x = "Absence ou Présence de Galumna", title = "Distribution de pa")
```

On voit ci-dessus plus précisément que 45 coeurs ne contiennent pas de Galumna et 25 coeurs en contiennent.

```{r, echo=F}
ggplot(data=mites) + geom_histogram(aes(prop), fill="darkred", bins = 30) +labs(y = "Nombre de coeurs", x = "Proportion de Galumna", title = "Distribution de prop")
```
...
```{r, echo=F}
ggplot(data=mites) + geom_histogram(aes(SubsDens), fill="darkred", binwidth = 5) +labs(y = "Nombre de coeurs", x = "SubsDens", title = "Distribution de SubsDens")
```

```{r, echo=F}
ggplot(data=mites) + geom_histogram(aes(WatrCont), fill="darkred", binwidth = 40) +labs(y = "Nombre de coeurs", x = "Watrcont", title = "Distribution de WatrCont")
```
...
```{r, echo=F}
ggplot(data = mites) + geom_bar(mapping = aes(Substrate), fill="darkred")  +labs(y = "Nombre de coeurs", x = "Substrate", title = "Distribution de Substrate")
```
...
```{r, echo=F}
ggplot(data = mites) + geom_bar(mapping = aes(Shrub), fill="darkred")  +labs(y = "Nombre de coeurs", x = "Shrub", title = "Distribution de Shrub")
```
...
```{r, echo=F}
ggplot(data = mites) + geom_bar(mapping = aes(Topo), fill="darkred")  +labs(y = "Nombre de coeurs", x = "Topo", title = "Distribution de Topo")
```
...
ggplot(data=data.frame(mites$prop, prop.asin),aes(x=mites$prop, y=prop.asin)) + geom_point() + geom_line(color="darkred")+ labs(title="Transformation Arcsin",x = "prop", y="Arcsin(prop)") + theme(plot.title = element_text(size = 10, color = "darkred", face = "bold"))
```
...
2.  un vecteur de paramètre $\beta$ à estimer et des variables explictives $X=[1,x^1,…,x^n]$
...
$$F=\frac{n-p}{q} \frac{SCR_c-SCR}{SCR} \sim F(q,n-p)$$ où $SCR$ est la somme des carrés des résidus dans le modèle complet et $SCR_c$ la somme des carrés des résidus du sous-modèle.
...
Nous effectuons une sélection stepwise descendante, toujours avec le critère BIC (option `k=log(n)` dans la fonction `step`). Celle-ci part du plus gros modèle et tente d'enlever les varaibles les moins significtives afin de retrouver le meilleur modèle.
...
```{r, echo=F}
kable(data.frame(exp(glm.best.pa$coefficients),row.names=c("Intercept", "WatrCont", "SubsDens", "TopoHummock")), col.names=c("Odds-ratios"))
```
...
-   **Evaluation de l'ajustement du modèle**
    ...
    n <- nrow(mites)
    plot(influence(glm.best.pa)$hat, type="h", ylab="h_ii", main="Points leviers")
    ...
    ```{r, echo=F}
    plot(cooks.distance(glm.best.pa), type="h", ylab="Distance de Cook", main="Points influents")
    ```
    ...
    plot(res2, ylab="Residuals", main="Résidus de Pearson")
    ...
    Par ailleurs, on peut également effectuer un test d'Hosmer-Lemeshow permettant d'évaluer la pertinence d'un modèle de régression logistique pour lequel on a des données individuelles (pour plus d'informations voir *Hosmer D.W., Lemeshow S., 2000*)
    ...
    problème De même, la très grande p-value nous indique que le modèle est bien adapté aux données.
...
plot(res.err, main="Taux d'erreur en fonction du seuil")
```
...
Ainsi, nous en déduisons que le modèle \@ref(eq:a) de régression logistique expliquant l'occurrence de mites grâce au contenu en eau, à la densité du substrat et à la topographie obtient de bons résultats, tant sur son ajustement que sur son pouvoir de classification.
...
```{r}
kable(data.frame(exp(prop.reg.best$coefficients),row.names=c("Intercept", "WatrCont", "SubsDens", "TopoHummock")), col.names=c("Odds-ratios"))
```
...
-   **Evaluation de l'ajustement du modèle**
    ...
    plot(influence(prop.reg.back)$hat, type="h", ylab="h_ii", main="Points leviers")
    abline(h=c(2*p/n, 3*p/n), col=2)
    ```
...
Ainsi, nous en déduisons que le modèle \@ref(eq:b) de régression logistique expliquant la proportion de mites Galumna grâce au contenu en eau, à la densité du substrat et à la topographie obtient de bons résultats.
...
Avec pour le modèle 1, le modèle complet, pour le modèle 2, le modèle avec les variables `WatrCont` et `SubsDens` et le modèle 3, le modèle selectionné par la sélection backward.
...
Le modele selectionné s'écrit donc : $$log(Y)=\beta_0+\beta_1*WatrCont+\beta_2*SubsDens+\beta_3*TopoHummock$$ ce qui nous donne à l'aide du `summary` de notre modèle : 
\begin{equation}
...
Nous allons maintenant interpréter les coefficients de notre modèle. Nous allons determiner l'effet de chaque coefficient du modèle sur le prédicteur linéaire $\mu_i=\beta_0+\beta_1*WatrCont+\beta_2*SubsDens+\beta_3*TopoHummock$, puis déduire l'effet sur la moyenne de réponse à partir de la fonction $\lambda=e^\eta$.
...
-   **Évaluation de l'ajustement du modèle**
    ...
    n <- nrow(mites)
    plot(influence(glm.poi4)$hat, type="h", ylab="h_ii", main="Points leviers")
    abline(h=c(2*p/n, 3*p/n), col=2)
    ```
    ...
    ```{r, echo=F}
    plot(cooks.distance(glm.poi4), type="h", ylab="Distance de Cook", main="Points influents")
    ```
    ...
    Problème très petite p-val (idem à part 3)
...
Ainsi, nous en déduisons que le modèle \@ref(eq:c) de régression Poisson expliquant l'abondance de Galumna grâce au contenu en eau, à la densité du substrat et à la topographie  est un bon modèle explicatif pour nos données.
...
Enfin, nous utilisons le mécanisme MNAR. Cette fois-ci, les données sont manquantes de façon non aléatoire, c'est-à-dire qu'elles dépendent des variables en question. En effet, la probabilité qu'une valeur soit manquante dépend d'une ou plusieurs données non observées pour les autres variables, ici $X$. Dans notre cas, on a $P(M=0\vert{Y})=\mathcal{B}(\phi(1.2X-0.5))$, où $\phi(.)$ désigne toujourds la fonction de répartition d'une loi normale $\mathcal{N}(0,1)$.
...
Nous pouvons alors regarder plus en détails comment sont reparties les données manquantes de $Y$ selon les valeurs de $X$ pour chacun des jeux de données.

```{r}
ggplot(data = don.ismcar) +
aes(x = X, y = Y) + geom_miss_point()
...
Pour ce faire, plusieurs méthodes seront utilisées. En effet, dans un premier temps, nous allons utiliser uniquement les données non manquantes de la variable. Puis, nous testerons différentes méthodes d'imputation pour les valeurs non rensignées.
...
Nous pouvons à présent calculer la moyenne de $Y$. Pour cela, nous réalisons une régression simple sur la variable et nous récupérons l'intercept.
...
kable(data.frame(mean.mcar.cc,mean.mar.cc,mean.mnar.cc), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, caption = "Moyenne de Y")
```

On remarque que les moyennes obtenues sont très différentes selon les jeux de données. Seule la première se rapproche de $0$, la vraie valeur.
...
Pour éviter de supprimer les obsevartions où se trouvent des données manquantes, on peut décider d'imputer des valeurs à ces endroits. Plusieurs méthodes existent pour déterminer la valeur à indiquer. La première que nous testons consiste à remplacer le Na par la moyenne de la variable. Toutes les données manquantes seront donc "remplacées" par la même valeur.
...
kable(data.frame(mean.mcar.im,mean.mar.im,mean.mnar.im), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, caption = "Moyenne de Y")
```
...
kable(data.frame(mean.mcar.irs,mean.mar.irs,mean.mnar.irs), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, caption = "Moyenne de Y")
```
...
La méthode précédente ayant des résultats encourageants, nous souhaitons poursuivre son étude. En effet, une valeur unique ne pouvant pas refléter l'incertitude sur la prédiction, nous allons tester de renouveller vingt fois l'imputation.

```{r}
don.ismcar.irs20 <- complete(mice(don.ismcar, method = "norm.nob", m = 20, print = FALSE))
don.ismar.irs20 <- complete(mice(don.ismar, method = "norm.nob", m = 20, print = FALSE))
don.ismnar.irs20 <- complete(mice(don.ismnar, method = "norm.nob", m = 20, print = FALSE))

lm.mcar.irs20 <- lm(don.ismcar.irs20$Y~1)
lm.mar.irs20 <- lm(don.ismar.irs20$Y~1)
lm.mnar.irs20 <- lm(don.ismnar.irs20$Y~1)

mean.mcar.irs20 <- lm.mcar.irs20$coefficients
mean.mar.irs20 <- lm.mar.irs20$coefficients
mean.mnar.irs20 <- lm.mnar.irs20$coefficients

kable(data.frame(mean.mcar.irs20,mean.mar.irs20,mean.mnar.irs20), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, caption = "Moyenne de Y")
```

Une nouvelle fois, le mécanisme MAR obtient le meilleur résultat. Nous remarquons que les valeurs sont légèrement plus éloignées de $0$. Cependant, elles incorporent l'incertidude de l'imputation des données manquantes.
...
Enfin, nous allons tester de réaliser une nouvelle fois vingt imputations, mais cette fois-ci par régression linéaire.

```{r}
don.ismcar.ir20 <- complete(mice(don.ismcar, method = "norm", m = 20, print = FALSE))
don.ismar.ir20 <- complete(mice(don.ismar, method = "norm", m = 20, print = FALSE))
don.ismnar.ir20 <- complete(mice(don.ismnar, method = "norm", m = 20, print = FALSE))

lm.mcar.ir20 <- lm(don.ismcar.ir20$Y~1)
lm.mar.ir20 <- lm(don.ismar.ir20$Y~1)
lm.mnar.ir20 <- lm(don.ismnar.ir20$Y~1)

mean.mcar.ir20 <- lm.mcar.ir20$coefficients
mean.mar.ir20 <- lm.mar.ir20$coefficients
mean.mnar.ir20 <- lm.mnar.ir20$coefficients

kable(data.frame(mean.mcar.ir20,mean.mar.ir20,mean.mnar.ir20), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, caption = "Moyenne de Y")
```

C'est encore pour le deuxième jeu de données que lon obtient le meilleur résultat. L'imputation par régression semble donc mieux adaptée pour celui-ci.

Regardons si cela ce confirme pour les intervalles de confiance associés aux moyennes :
...
***NOTE** : J'ai limpression de me répéter de ouf, je sais pas quoi dire d'autres.*


# Simulation de données manquantes sur le jeu de données `mites`.
...
Sites importants :
https://delladata.fr/decrire-description-descriptives/ 
https://mran.microsoft.com/snapshot/2017-04-24/web/packages/mice/vignettes/ampute.html

Explorer les 3 dispositifs de données manquantes créés.
On pourra utiliser la fonction `aggr` du package `VIM` pour l'analyse univariée, `CramerV` du package `DescTools` pour l'analyse bivariée, `MCA` du package `FactoMineR` pour l'analyse multidimensionnelle
...
La ligne entière bleue représente toutes les lignes dans le data frame qui ne comportent pas de données manquantes (90% des lignes).
Une case rouge représente toutes les lignes dans le data frame qui sont manquantes pour cette variable. Dans notre cas nous avons par exemple 2,9% pour la varible "Topo".
...
Plusieurs cases rouges représentent les observations pour lesquelles les valeurs dans le data frame sont manquantes conjointement pour les variables concernées. Par exemple, 2,9% des lignes, ont une donnée manquante à la fois sur les variables "Substrate", "Shrub", "Topo" et "Prop". 
On observe qu'il n'y a pas de ligne qui ne comportent pas de données manquantes, en comparaison avec le dispositif MCAR. Autrement dit, chaque ligne contient des données manquantes. 
...
tab <- table(amp.mcar$amp$Galumna, amp.mcar$amp$pa)
names(amp.mcar$amp)
CramerV(tab)
```

**Analyse muldimentionnelle**
`MCA` du package `FactoMineR` pour l'analyse multidimensionnelle
```{r}
library("FactoMineR")
?MCA

```
Possible de faire MCA avec les données manquantes ???
...
amp.mcar$amp %>% 
    tbl_summary() # Je comprends pas tout
```


### Imputer les données manquanntes 

Nous allons maintenant imputer les données manquantes sur les 3 dispositifs que nous avons crés précédement. 

**Imputation simple**
(Moyenne, médiane, mode, LOVC, kNN, loess, SVD, missForests -> faire les méthodes adaptées)

#1. Par la moyenne et la médiane

Il me semble que on ne peux pas faire l'imputation par la moyenne et la médiane car nous avons des variables qualitatives ce n'est donc pas possible. A vérifier mais cela me parait logique de pas pouvoir faire de moyenne sur une variable qualitative comme `Topo`. Cela remplace avec des valeurs numériques au lieu de valeurs qualitatives.
...
library(Hmisc)
?impute
dat.moy=impute(amp.mar$amp, fun=mean)
as.matrix(dat.moy)
```
