...
**MCAR** Nous avons vu que dans le dispositif MCAR, les variables `SubsDens` et `prop` sont les deux variables ayant le plus de données manquantes. Nous allons donc voir si ces 2 variables ont une forte relation. Nous allons de plus, tester sur différentes paires de variables.
...
Le nom des variable finissant par "-m" signifie les données manquantes pour cette variables, contrairement au nom des variables se termiant par "-o".
...
Nous utilisons aussi la fonction `impute` du package `Hmisc`. **MCAR**

```{r}
dat.med.mcar=impute(amp.mcar$amp, fun=median)
as.matrix(dat.med.mcar)
```
...
Avec cette méthode nous remarquons que la variable `Galumna` et `totalabund` ne sont plus des valeurs entières. Nous prenons donc pas cette méthode en considération dans notre analyse.
...
Comme pour la méthode Loess, nous remarquons que la variable `Galumna` et `totalabund` ne sont plus des valeurs entières.
