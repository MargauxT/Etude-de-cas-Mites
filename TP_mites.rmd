...
```{r, echo = F, fig.height=4.5,fig.cap= "Evolution de la moyenne de Y apr§s imputation multiple par régression"}
...
Nous utiliserons la fonction `MCA` du package `FactoMineR` pour l'analyse multidimensionnelle. L'analyse des correspondances multiples est une technique descriptive visant à résumer l'information contenu dans un grand nombre de variables afin de faciliter l'interprétention des corrélations existantes entre ces différentes variables. On cherche à savoir quelles sont les variables corrélées entre elles. L'analyse se concentre sur ses deux premiers axes qui constitueront un bon résumé des variations observables dans le jeu de données. Le nom des variable finissant par "-m" signifie les données manquantes pour cette variable, contrairement au nom des variables se termiant par "-o".

```{r, echo = F, fig.height=5, fig.cap= "MCAR"}
par(mfrow = c(2,2))
...
```{r, echo = F, fig.height=5, fig.cap= "MAR"}
par(mfrow = c(2,2))
...
```{r, echo = F, fig.height=5, fig.cap= "MNAR"}
par(mfrow = c(2,2))
...
```
Les données imputées sont en rouges. On remarque des différences dans les 3 dispositifs. En effet, dans le dispositif MNAR et plus particulierement pour la variable `prop`, les données imputées ne respectent pas les données initiales en comparaison aux dispositifs MCAR et MAR pour la variable `prop` qui respecte bien le distribution des données initiales. Les deux dispositifs MCAR et MAR respecte assez bien la distribution des données initiales. 
...
Pour la variable `Galumna`, le resultat n'est pas similaire, dans ce cas le dispositif MNAR a le meilleur résultat. 
...
Pour ces $3$ variables différentes ils n'y a pas de dispositif qui se démarquent des autres. Cependant, le dispositif MAR semble être plutot bon sur chaque variable tandis que par exemple le dispositif MNAR est mauvais pour la variable `prop`. Nous retenons donc le dispositif MAR pour l'imputation multiple par régression stochastique. 

-   Imputation multiple par régression 

Nous allons une nouvelle fois tester une imputation multiple mais cette fois-ci par régression linéaire. Nous indiquons donc dans la fonction `mice` l'argument `method=norm`. 

```{r, echo = F, fig.height=4, fig.cap= "MCAR - Imputation multiple par régression stochastique"}
set.seed(123)
mcar.mice1 <- mice(amp.mcar$amp, method = "norm", m = 20, print = FALSE)
mar.mice1 <- mice(amp.mar$amp, method = "norm", m = 20, print = FALSE)
mnar.mice1 <- mice(amp.mnar$amp, method = "norm", m = 20, print = FALSE)

stripplot(mcar.mice1, pch = 20, cex = 1, cex.main = 1, main = "MCAR")
```
```{r, echo = F, fig.height=4, fig.cap= "MAR - Imputation multiple par régression stochastique"}
stripplot(mar.mice1, pch = 20, cex = 1, cex.main = 1, main = "MAR")
```
```{r, echo = F, fig.height=4, fig.cap= "MNAR - Imputation multiple par régression stochastique"}
stripplot(mnar.mice1, pch = 20, cex = 1, cex.main = 1, main = "MNAR")
```
Nous calculons ensuite les moyennes pour les 3 variables étudiées précedemment. 

```{r, echo = F}
set.seed(123)

fit_mcar.mice1 <- with(mcar.mice1, lm(prop~1))
fitpool_mcar.mice1 <- pool(fit_mcar.mice1)
mean.mcar.mice1 <- fitpool_mcar.mice1$pooled[3]

fit_mar.mice1 <- with(mar.mice1, lm(prop~1))
fitpool_mar.mice1 <- pool(fit_mar.mice1)
mean.mar.mice1 <- fitpool_mar.mice1$pooled[3]

fit_mnar.mice1 <- with(mnar.mice1, lm(prop~1))
fitpool_mnar.mice1 <- pool(fit_mnar.mice1)
mean.mnar.mice1 <- fitpool_mnar.mice1$pooled[3]

mean(mites1$prop)
kable(data.frame(mean.mcar.mice1,mean.mar.mice1,mean.mnar.mice1), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, position="H", caption = "Moyenne de prop après combinaison des 20 imputations par régression stochastique")
```

```{r, echo = F}
set.seed(123)

fit_mcar.mice1 <- with(mcar.mice1, lm(Galumna~1))
fitpool_mcar.mice1 <- pool(fit_mcar.mice1)
mean.mcar.mice1 <- fitpool_mcar.mice1$pooled[3]

fit_mar.mice1 <- with(mar.mice1, lm(Galumna~1))
fitpool_mar.mice1 <- pool(fit_mar.mice1)
mean.mar.mice1 <- fitpool_mar.mice1$pooled[3]

fit_mnar.mice1 <- with(mnar.mice1, lm(Galumna~1))
fitpool_mnar.mice1 <- pool(fit_mnar.mice1)
mean.mnar.mice1 <- fitpool_mnar.mice1$pooled[3]

mean(mites1$Galumna)
kable(data.frame(mean.mcar.mice1,mean.mar.mice1,mean.mnar.mice1), col.names = c("MCAR", "MAR", "MNAR"), row.names = FALSE, position="H", caption = "Moyenne de Galumna après combinaison des 20 imputations par régression stochastique")
```

Nous obtenons les mêmes résultats pour l'imputation multiple par régression. 

**Conclusion**

Nous avons utilisé le jeu de données `mites` pour générer des données manquantes suivant les 3 catégories MCAR, MAR et MNAR. Nous avons ensuite analyser ces 3 dispositifs dans une analyse univariée, une analyse bivariée et une analyse muldimentionnelle. Pour finir, nous avons imputer les données manquantes de plusieurs méthodes différentes, de l'imputation simple à l'imputation multiple. 
