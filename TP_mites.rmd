...
knitr::opts_chunk$set(echo = TRUE, fig.width = 7, fig.height = 4, fig.pos="H", out.extra = "")
library(lmtest)
library(stats)
library(car)
library(MASS)
...
```{r, echo=F, warning=F, fig.cap="Variances des résidus des modèles de régression linéaire"}
par(mfrow=c(2,2))
...
```{r, echo=F, warning =F, fig.cap="QQ-plot des modèles de régression linéaire"}
par(mfrow=c(2,2))
...
```{r, echo=F, fig.cap="Corrélations avec la variable SubsDens"}
...
```{r, echo=F, fig.cap="Corrélations avec la variable WatrCont"}
...
```{r, echo=F, fig.cap="Corrélations avec la variable Substrate"}
